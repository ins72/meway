...
class ComprehensiveBackendTester:
    ...
    async def test_stripe_integration(self) -> Dict[str, Any]:
        ...
        results["total"] += 1
        try:
            async with self.session.get(f"{self.api_url}/stripe/health") as response:
                if response.status == 200:
                    data = await response.json()
                    results["passed"] += 1
        ...
        try:
            ...
            async with self.session.post(f"{self.api_url}/stripe/payment-intent", json=payment_data, headers=headers) as response:
                if response.status == 200 or response.status == 201:
                    data = await response.json()
                    results["passed"] += 1
