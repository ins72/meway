...
from services.ai_token_management_service import get_ai_token_management_service
import logging
...
async def health_check():
    ...
    try:
        service = get_ai_token_management_service()
...
@router.post("/")
async def create_ai_token_management(
    data: Dict[str, Any] = Body({}, description="Data for creating ai_token_management"),
    ...
    try:
        ...
        service = get_ai_token_management_service()
        result = await service.create_ai_token_management(data)
...
@router.get("/")
async def list_ai_token_managements(
    ...
    try:
        service = get_ai_token_management_service()
        result = await service.list_ai_token_managements(
...
@router.get("/{item_id}")
async def get_ai_token_management(
    item_id: str = Path(..., description="ID of ai_token_management"),
    current_user: dict = Depends(get_current_admin)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_ai_token_management_service()
        result = await service.get_ai_token_management(item_id)
...
@router.put("/{item_id}")
async def update_ai_token_management(
    item_id: str = Path(..., description="ID of ai_token_management"),
    ...
    try:
        ...
        service = get_ai_token_management_service()
        result = await service.update_ai_token_management(item_id, data)
...
async def delete_ai_token_management(
    item_id: str = Path(..., description="ID of ai_token_management"),
    current_user: dict = Depends(get_current_admin)
):
    ...
    try:
        service = get_ai_token_management_service()
        result = await service.delete_ai_token_management(item_id)
