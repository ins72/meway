...
class ContentCreate(BaseModel):
    name: str = Field(..., min_length=1, max_length=255, description="Name of the content")
    description: Optional[str] = Field(None, max_length=1000, description="Description of the content")
    status: Optional[str] = Field("active", description="Status of the content")
...
class ContentUpdate(BaseModel):
    name: Optional[str] = Field(None, min_length=1, max_length=255, description="Name of the content")
    description: Optional[str] = Field(None, max_length=1000, description="Description of the content")
    status: Optional[str] = Field(None, description="Status of the content")
    metadata: Optional[Dict[str, Any]] = Field(None, description="Additional metadata")
...
async def health_check():
    """Comprehensive health check for content service"""
    try:
...
async def create_content(
    ...
    try:
        ...
        service = get_content_service()
        result = await service.create_content(item_data, user_id=user_id)
        ...
        logger.error(f"Create content failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def list_contents(
    ...
    """List contents with comprehensive filtering and pagination"""
    try:
        user_id = current_user.get("id") or current_user.get("user_id")
        ...
        if search:
            service = get_content_service()
            result = await service.search_contents(
                search_query=search,
                user_id=user_id,
                limit=limit,
                offset=offset
        ...
        logger.error(f"List contents failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@router.get("/{item_id}", response_model=ContentResponse)
async def get_content(
    item_id: str = Path(..., description="ID of the content to retrieve"),
    current_user: dict = Depends(get_current_user)
):
    """Get content by ID with comprehensive error handling"""
    try:
        user_id = current_user.get("id") or current_user.get("user_id")
        
        service = get_content_service()
        result = await service.get_content(item_id, user_id=user_id)
...
async def update_content(
    ...
    try:
        ...
        service = get_content_service()
        result = await service.update_content(item_id, item_data, user_id=user_id)
        ...
        logger.error(f"Update content failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))

# Delete Operation
@router.delete("/{item_id}", response_model=ContentResponse)
async def delete_content(
    item_id: str = Path(..., description="ID of the content to delete"),
    permanent: bool = Query(False, description="Permanent delete (true) or soft delete (false)"),
    current_user: dict = Depends(get_current_user)
):
    """Delete content with comprehensive validation"""
    try:
        user_id = current_user.get("id") or current_user.get("user_id")
...
# Statistics
@router.get("/stats", response_model=ContentResponse)
async def get_content_stats(
    current_user: dict = Depends(get_current_user)
):
    """Get comprehensive statistics for contents"""
    try:
        user_id = current_user.get("id") or current_user.get("user_id")
        ...
        logger.error(f"Get content stats failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def bulk_create_contents(
    ...
    try:
        ...
        service = get_content_service()
        result = await service.bulk_create_contents(items_data, user_id=user_id)
        ...
        logger.error(f"Bulk create contents failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))