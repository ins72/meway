...
async def create_i18n(
    data: Dict[str, Any] = Body({}, description="Data for creating i18n"),
    ...
    try:
        ...
        service = get_i18n_service()
        result = await service.create_i18n(data)
...
async def list_i18ns(
    ...
    try:
        service = get_i18n_service()
        result = await service.list_i18ns(
...
@router.get("/{item_id}")
async def get_i18n(
    item_id: str = Path(..., description="ID of i18n"),
    current_user: dict = Depends(get_current_admin)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_i18n_service()
        result = await service.get_i18n(item_id)
...
async def update_i18n(
    ...
    try:
        ...
        service = get_i18n_service()
        result = await service.update_i18n(item_id, data)
...
async def delete_i18n(
    ...
    try:
        service = get_i18n_service()
        result = await service.delete_i18n(item_id)
