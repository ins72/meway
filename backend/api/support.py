...
async def create_support(
    data: Dict[str, Any] = Body({}, description="Data for creating support"),
    ...
    try:
        ...
        service = get_support_service()
        result = await service.create_support(data)
...
async def get_support(
    item_id: str = Path(..., description="ID of support"),
    current_user: dict = Depends(get_current_admin)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_support_service()
        result = await service.get_support(item_id)
...
async def update_support(
    ...
    try:
        ...
        service = get_support_service()
        result = await service.update_support(item_id, data)
...
async def delete_support(
    ...
    try:
        service = get_support_service()
        result = await service.delete_support(item_id)
