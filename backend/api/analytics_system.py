...
from services.analytics_system_service import get_analytics_system_service
import logging
...
async def health_check():
    ...
    try:
        service = get_analytics_system_service()
...
async def create_analytics_system(
    data: Dict[str, Any] = Body({}, description="Data for creating analytics_system"),
    ...
    try:
        ...
        service = get_analytics_system_service()
        result = await service.create_analytics_system(data)
...
@router.get("/")
async def list_analytics_systems(
    ...
    try:
        service = get_analytics_system_service()
        result = await service.list_analytics_systems(
...
async def get_analytics_system(
    item_id: str = Path(..., description="ID of analytics_system"),
    current_user: dict = Depends(get_current_user)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_analytics_system_service()
        result = await service.get_analytics_system(item_id)
...
@router.put("/{item_id}")
async def update_analytics_system(
    item_id: str = Path(..., description="ID of analytics_system"),
    ...
    try:
        ...
        service = get_analytics_system_service()
        result = await service.update_analytics_system(item_id, data)
...
async def delete_analytics_system(
    item_id: str = Path(..., description="ID of analytics_system"),
    current_user: dict = Depends(get_current_user)
):
    ...
    try:
        service = get_analytics_system_service()
        result = await service.delete_analytics_system(item_id)
