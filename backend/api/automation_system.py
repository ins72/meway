...
from services.automation_system_service import get_automation_system_service
import logging
...
async def health_check():
    ...
    try:
        service = get_automation_system_service()
...
@router.post("/")
async def create_automation_system(
    data: Dict[str, Any] = Body({}, description="Data for creating automation_system"),
    ...
    try:
        ...
        service = get_automation_system_service()
        result = await service.create_automation_system(data)
...
@router.get("/")
async def list_automation_systems(
    ...
    try:
        service = get_automation_system_service()
        result = await service.list_automation_systems(
...
@router.get("/{item_id}")
async def get_automation_system(
    item_id: str = Path(..., description="ID of automation_system"),
    current_user: dict = Depends(get_current_admin)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_automation_system_service()
        result = await service.get_automation_system(item_id)
...
@router.put("/{item_id}")
async def update_automation_system(
    item_id: str = Path(..., description="ID of automation_system"),
    ...
    try:
        ...
        service = get_automation_system_service()
        result = await service.update_automation_system(item_id, data)
...
async def delete_automation_system(
    item_id: str = Path(..., description="ID of automation_system"),
    current_user: dict = Depends(get_current_admin)
):
    ...
    try:
        service = get_automation_system_service()
        result = await service.delete_automation_system(item_id)
