...
from services.mobile_pwa_service import get_mobile_pwa_service
import logging
...
async def health_check():
    ...
    try:
        service = get_mobile_pwa_service()
...
@router.post("/")
async def create_mobile_pwa(
    data: Dict[str, Any] = Body({}, description="Data for creating mobile_pwa"),
    ...
    try:
        ...
        service = get_mobile_pwa_service()
        result = await service.create_mobile_pwa(data)
...
async def list_mobile_pwas(
    ...
    try:
        service = get_mobile_pwa_service()
        result = await service.list_mobile_pwas(
...
async def get_mobile_pwa(
    item_id: str = Path(..., description="ID of mobile_pwa"),
    current_user: dict = Depends(get_current_admin)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_mobile_pwa_service()
        result = await service.get_mobile_pwa(item_id)
...
async def update_mobile_pwa(
    ...
    try:
        ...
        service = get_mobile_pwa_service()
        result = await service.update_mobile_pwa(item_id, data)
...
async def delete_mobile_pwa(
    ...
    try:
        service = get_mobile_pwa_service()
        result = await service.delete_mobile_pwa(item_id)
