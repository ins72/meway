...
from services.real_email_automation_service import get_real_email_automation_service
import logging
...
async def health_check():
    ...
    try:
        service = get_real_email_automation_service()
...
@router.post("/")
async def create_real_email_automation(
    data: Dict[str, Any] = Body({}, description="Data for creating real_email_automation"),
    ...
    try:
        ...
        service = get_real_email_automation_service()
        result = await service.create_real_email_automation(data)
...
@router.get("/")
async def list_real_email_automations(
    ...
    try:
        service = get_real_email_automation_service()
        result = await service.list_real_email_automations(
...
@router.get("/{item_id}")
async def get_real_email_automation(
    item_id: str = Path(..., description="ID of real_email_automation"),
    current_user: dict = Depends(get_current_admin)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_real_email_automation_service()
        result = await service.get_real_email_automation(item_id)
...
@router.put("/{item_id}")
async def update_real_email_automation(
    item_id: str = Path(..., description="ID of real_email_automation"),
    ...
    try:
        ...
        service = get_real_email_automation_service()
        result = await service.update_real_email_automation(item_id, data)
...
async def delete_real_email_automation(
    item_id: str = Path(..., description="ID of real_email_automation"),
    current_user: dict = Depends(get_current_admin)
):
    ...
    try:
        service = get_real_email_automation_service()
        result = await service.delete_real_email_automation(item_id)
