...
async def create_email_marketing(
    data: Dict[str, Any] = Body({}, description="Data for creating email_marketing"),
    ...
    try:
        ...
        service = get_email_marketing_service()
        result = await service.create_email_marketing(data)
...
async def get_email_marketing(
    item_id: str = Path(..., description="ID of email_marketing"),
    current_user: dict = Depends(get_current_admin)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_email_marketing_service()
        result = await service.get_email_marketing(item_id)
...
async def update_email_marketing(
    ...
    try:
        ...
        service = get_email_marketing_service()
        result = await service.update_email_marketing(item_id, data)
...
async def delete_email_marketing(
    ...
    try:
        service = get_email_marketing_service()
        result = await service.delete_email_marketing(item_id)
