...
from services.link_shortener_service import get_link_shortener_service
import logging
...
async def health_check():
    ...
    try:
        service = get_link_shortener_service()
...
@router.post("/")
async def create_link_shortener(
    data: Dict[str, Any] = Body({}, description="Data for creating link_shortener"),
    ...
    try:
        ...
        service = get_link_shortener_service()
        result = await service.create_link_shortener(data)
...
@router.get("/")
async def list_link_shorteners(
    ...
    try:
        service = get_link_shortener_service()
        result = await service.list_link_shorteners(
...
@router.get("/{item_id}")
async def get_link_shortener(
    item_id: str = Path(..., description="ID of link_shortener"),
    current_user: dict = Depends(get_current_user)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_link_shortener_service()
        result = await service.get_link_shortener(item_id)
...
@router.put("/{item_id}")
async def update_link_shortener(
    item_id: str = Path(..., description="ID of link_shortener"),
    ...
    try:
        ...
        service = get_link_shortener_service()
        result = await service.update_link_shortener(item_id, data)
...
async def delete_link_shortener(
    item_id: str = Path(..., description="ID of link_shortener"),
    current_user: dict = Depends(get_current_user)
):
    ...
    try:
        service = get_link_shortener_service()
        result = await service.delete_link_shortener(item_id)
