...
from services.escrow_system_service import get_escrow_system_service
import logging
...
async def health_check():
    ...
    try:
        service = get_escrow_system_service()
...
@router.post("/")
async def create_escrow_system(
    data: Dict[str, Any] = Body({}, description="Data for creating escrow_system"),
    ...
    try:
        ...
        service = get_escrow_system_service()
        result = await service.create_escrow_system(data)
...
async def list_escrow_systems(
    ...
    try:
        service = get_escrow_system_service()
        result = await service.list_escrow_systems(
...
@router.get("/{item_id}")
async def get_escrow_system(
    item_id: str = Path(..., description="ID of escrow_system"),
    current_user: dict = Depends(get_current_user)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_escrow_system_service()
        result = await service.get_escrow_system(item_id)
...
@router.put("/{item_id}")
async def update_escrow_system(
    item_id: str = Path(..., description="ID of escrow_system"),
    ...
    try:
        ...
        service = get_escrow_system_service()
        result = await service.update_escrow_system(item_id, data)
...
async def delete_escrow_system(
    item_id: str = Path(..., description="ID of escrow_system"),
    current_user: dict = Depends(get_current_user)
):
    ...
    try:
        service = get_escrow_system_service()
        result = await service.delete_escrow_system(item_id)
