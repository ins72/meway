...
from services.support_system_service import get_support_system_service
import logging
...
async def health_check():
    ...
    try:
        service = get_support_system_service()
...
async def create_support_system(
    data: Dict[str, Any] = Body({}, description="Data for creating support_system"),
    ...
    try:
        ...
        service = get_support_system_service()
        result = await service.create_support_system(data)
...
@router.get("/")
async def list_support_systems(
    ...
    try:
        service = get_support_system_service()
        result = await service.list_support_systems(
...
@router.get("/{item_id}")
async def get_support_system(
    item_id: str = Path(..., description="ID of support_system"),
    current_user: dict = Depends(get_current_admin)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_support_system_service()
        result = await service.get_support_system(item_id)
...
@router.put("/{item_id}")
async def update_support_system(
    item_id: str = Path(..., description="ID of support_system"),
    ...
    try:
        ...
        service = get_support_system_service()
        result = await service.update_support_system(item_id, data)
...
async def delete_support_system(
    item_id: str = Path(..., description="ID of support_system"),
    current_user: dict = Depends(get_current_admin)
):
    ...
    try:
        service = get_support_system_service()
        result = await service.delete_support_system(item_id)
