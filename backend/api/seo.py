...
async def create_seo(
    data: Dict[str, Any] = Body({}, description="Data for creating seo"),
    ...
    try:
        ...
        service = get_seo_service()
        result = await service.create_seo(data)
...
async def list_seos(
    ...
    try:
        service = get_seo_service()
        result = await service.list_seos(
...
@router.get("/{item_id}")
async def get_seo(
    item_id: str = Path(..., description="ID of seo"),
    current_user: dict = Depends(get_current_admin)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_seo_service()
        result = await service.get_seo(item_id)
...
async def update_seo(
    ...
    try:
        ...
        service = get_seo_service()
        result = await service.update_seo(item_id, data)
...
async def delete_seo(
    ...
    try:
        service = get_seo_service()
        result = await service.delete_seo(item_id)
