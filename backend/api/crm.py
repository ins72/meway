...
async def create_crm(
    data: Dict[str, Any] = Body({}, description="Data for creating crm"),
    ...
    try:
        ...
        service = get_crm_service()
        result = await service.create_crm(data)
...
@router.get("/{item_id}")
async def get_crm(
    item_id: str = Path(..., description="ID of crm"),
    current_user: dict = Depends(get_current_admin)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_crm_service()
        result = await service.get_crm(item_id)
...
async def update_crm(
    ...
    try:
        ...
        service = get_crm_service()
        result = await service.update_crm(item_id, data)
...
async def delete_crm(
    ...
    try:
        service = get_crm_service()
        result = await service.delete_crm(item_id)
