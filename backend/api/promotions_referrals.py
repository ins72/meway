...
from services.promotions_referrals_service import get_promotions_referrals_service
import logging
...
async def health_check():
    ...
    try:
        service = get_promotions_referrals_service()
...
@router.post("/")
async def create_promotions_referrals(
    data: Dict[str, Any] = Body({}, description="Data for creating promotions_referrals"),
    ...
    try:
        ...
        service = get_promotions_referrals_service()
        result = await service.create_promotions_referrals(data)
...
@router.get("/")
async def list_promotions_referralss(
    ...
    try:
        service = get_promotions_referrals_service()
        result = await service.list_promotions_referralss(
...
@router.get("/{item_id}")
async def get_promotions_referrals(
    item_id: str = Path(..., description="ID of promotions_referrals"),
    current_user: dict = Depends(get_current_user)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_promotions_referrals_service()
        result = await service.get_promotions_referrals(item_id)
...
@router.put("/{item_id}")
async def update_promotions_referrals(
    item_id: str = Path(..., description="ID of promotions_referrals"),
    ...
    try:
        ...
        service = get_promotions_referrals_service()
        result = await service.update_promotions_referrals(item_id, data)
...
async def delete_promotions_referrals(
    item_id: str = Path(..., description="ID of promotions_referrals"),
    current_user: dict = Depends(get_current_user)
):
    ...
    try:
        service = get_promotions_referrals_service()
        result = await service.delete_promotions_referrals(item_id)
