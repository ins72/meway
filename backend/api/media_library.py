...
async def create_media_library(
    data: Dict[str, Any] = Body({}, description="Data for creating media_library"),
    ...
    try:
        ...
        service = get_media_library_service()
        result = await service.create_media_library(data)
...
async def list_media_librarys(
    ...
    try:
        service = get_media_library_service()
        result = await service.list_media_librarys(
...
async def get_media_library(
    item_id: str = Path(..., description="ID of media_library"),
    current_user: dict = Depends(get_current_admin)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_media_library_service()
        result = await service.get_media_library(item_id)
...
async def update_media_library(
    ...
    try:
        ...
        service = get_media_library_service()
        result = await service.update_media_library(item_id, data)
...
async def delete_media_library(
    ...
    try:
        service = get_media_library_service()
        result = await service.delete_media_library(item_id)
