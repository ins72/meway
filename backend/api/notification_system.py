...
from services.notification_system_service import get_notification_system_service
import logging
...
async def health_check():
    ...
    try:
        service = get_notification_system_service()
...
async def create_notification_system(
    data: Dict[str, Any] = Body({}, description="Data for creating notification_system"),
    ...
    try:
        ...
        service = get_notification_system_service()
        result = await service.create_notification_system(data)
...
@router.get("/")
async def list_notification_systems(
    ...
    try:
        service = get_notification_system_service()
        result = await service.list_notification_systems(
...
async def get_notification_system(
    item_id: str = Path(..., description="ID of notification_system"),
    current_user: dict = Depends(get_current_admin)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_notification_system_service()
        result = await service.get_notification_system(item_id)
...
@router.put("/{item_id}")
async def update_notification_system(
    item_id: str = Path(..., description="ID of notification_system"),
    ...
    try:
        ...
        service = get_notification_system_service()
        result = await service.update_notification_system(item_id, data)
...
async def delete_notification_system(
    item_id: str = Path(..., description="ID of notification_system"),
    current_user: dict = Depends(get_current_admin)
):
    ...
    try:
        service = get_notification_system_service()
        result = await service.delete_notification_system(item_id)
