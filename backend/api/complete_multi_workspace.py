...
from services.complete_multi_workspace_service import get_complete_multi_workspace_service
import logging
...
async def health_check():
    ...
    try:
        service = get_complete_multi_workspace_service()
...
@router.post("/")
async def create_complete_multi_workspace(
    data: Dict[str, Any] = Body({}, description="Data for creating complete_multi_workspace"),
    ...
    try:
        ...
        service = get_complete_multi_workspace_service()
        result = await service.create_complete_multi_workspace(data)
...
@router.get("/")
async def list_complete_multi_workspaces(
    ...
    try:
        service = get_complete_multi_workspace_service()
        result = await service.list_complete_multi_workspaces(
...
@router.get("/{item_id}")
async def get_complete_multi_workspace(
    item_id: str = Path(..., description="ID of complete_multi_workspace"),
    current_user: dict = Depends(get_current_admin)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_complete_multi_workspace_service()
        result = await service.get_complete_multi_workspace(item_id)
...
@router.put("/{item_id}")
async def update_complete_multi_workspace(
    item_id: str = Path(..., description="ID of complete_multi_workspace"),
    ...
    try:
        ...
        service = get_complete_multi_workspace_service()
        result = await service.update_complete_multi_workspace(item_id, data)
...
async def delete_complete_multi_workspace(
    item_id: str = Path(..., description="ID of complete_multi_workspace"),
    current_user: dict = Depends(get_current_admin)
):
    ...
    try:
        service = get_complete_multi_workspace_service()
        result = await service.delete_complete_multi_workspace(item_id)
