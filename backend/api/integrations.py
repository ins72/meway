...
from services.integrations_service import get_integrations_service
import logging
...
async def create_integrations(
    data: Dict[str, Any] = Body({}, description="Data for creating integrations"),
    ...
    try:
        ...
        service = get_integrations_service()
        result = await service.create_integrations(data)
...
async def list_integrationss(
    ...
    try:
        service = get_integrations_service()
        result = await service.list_integrationss(
...
async def get_integrations(
    item_id: str = Path(..., description="ID of integrations"),
    current_user: dict = Depends(get_current_admin)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_integrations_service()
        result = await service.get_integrations(item_id)
...
async def update_integrations(
    ...
    try:
        ...
        service = get_integrations_service()
        result = await service.update_integrations(item_id, data)
...
async def delete_integrations(
    ...
    try:
        service = get_integrations_service()
        result = await service.delete_integrations(item_id)
