...
from services.real_data_population_service import get_real_data_population_service
import logging
...
async def health_check():
    ...
    try:
        service = get_real_data_population_service()
...
@router.post("/")
async def create_real_data_population(
    data: Dict[str, Any] = Body({}, description="Data for creating real_data_population"),
    ...
    try:
        ...
        service = get_real_data_population_service()
        result = await service.create_real_data_population(data)
...
@router.get("/")
async def list_real_data_populations(
    ...
    try:
        service = get_real_data_population_service()
        result = await service.list_real_data_populations(
...
@router.get("/{item_id}")
async def get_real_data_population(
    item_id: str = Path(..., description="ID of real_data_population"),
    current_user: dict = Depends(get_current_user)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_real_data_population_service()
        result = await service.get_real_data_population(item_id)
...
@router.put("/{item_id}")
async def update_real_data_population(
    item_id: str = Path(..., description="ID of real_data_population"),
    ...
    try:
        ...
        service = get_real_data_population_service()
        result = await service.update_real_data_population(item_id, data)
...
async def delete_real_data_population(
    item_id: str = Path(..., description="ID of real_data_population"),
    current_user: dict = Depends(get_current_user)
):
    ...
    try:
        service = get_real_data_population_service()
        result = await service.delete_real_data_population(item_id)
