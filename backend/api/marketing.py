...
async def create_marketing(
    data: Dict[str, Any] = Body({}, description="Data for creating marketing"),
    ...
    try:
        ...
        service = get_marketing_service()
        result = await service.create_marketing(data)
...
async def list_marketings(
    ...
    try:
        service = get_marketing_service()
        result = await service.list_marketings(
...
async def get_marketing(
    item_id: str = Path(..., description="ID of marketing"),
    current_user: dict = Depends(get_current_admin)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_marketing_service()
        result = await service.get_marketing(item_id)
...
async def update_marketing(
    ...
    try:
        ...
        service = get_marketing_service()
        result = await service.update_marketing(item_id, data)
...
async def delete_marketing(
    ...
    try:
        service = get_marketing_service()
        result = await service.delete_marketing(item_id)
