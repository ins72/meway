...
async def create_settings(
    data: Dict[str, Any] = Body({}, description="Data for creating settings"),
    ...
    try:
        ...
        service = get_settings_service()
        result = await service.create_settings(data)
...
async def list_settingss(
    ...
    try:
        service = get_settings_service()
        result = await service.list_settingss(
...
async def get_settings(
    item_id: str = Path(..., description="ID of settings"),
    current_user: dict = Depends(get_current_user)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_settings_service()
        result = await service.get_settings(item_id)
...
async def update_settings(
    ...
    try:
        ...
        service = get_settings_service()
        result = await service.update_settings(item_id, data)
...
async def delete_settings(
    ...
    try:
        service = get_settings_service()
        result = await service.delete_settings(item_id)
