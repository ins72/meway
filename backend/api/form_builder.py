...
from services.form_builder_service import get_form_builder_service
import logging
...
async def health_check():
    ...
    try:
        service = get_form_builder_service()
...
@router.post("/")
async def create_form_builder(
    data: Dict[str, Any] = Body({}, description="Data for creating form_builder"),
    ...
    try:
        ...
        service = get_form_builder_service()
        result = await service.create_form_builder(data)
...
async def list_form_builders(
    ...
    try:
        service = get_form_builder_service()
        result = await service.list_form_builders(
...
@router.get("/{item_id}")
async def get_form_builder(
    item_id: str = Path(..., description="ID of form_builder"),
    current_user: dict = Depends(get_current_admin)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_form_builder_service()
        result = await service.get_form_builder(item_id)
...
async def update_form_builder(
    ...
    try:
        ...
        service = get_form_builder_service()
        result = await service.update_form_builder(item_id, data)
...
async def delete_form_builder(
    ...
    try:
        service = get_form_builder_service()
        result = await service.delete_form_builder(item_id)
