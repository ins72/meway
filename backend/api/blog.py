...
async def create_blog(
    data: Dict[str, Any] = Body({}, description="Data for creating blog"),
...
async def list_blogs(
    ...
    try:
        service = get_blog_service()
        result = await service.list_blogs(
...
@router.get("/{item_id}")
async def get_blog(
    item_id: str = Path(..., description="ID of blog"),
    current_user: dict = Depends(get_current_admin)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_blog_service()
        result = await service.get_blog(item_id)
...
async def update_blog(
    ...
    try:
        ...
        service = get_blog_service()
        result = await service.update_blog(item_id, data)
...
async def delete_blog(
    ...
    try:
        service = get_blog_service()
        result = await service.delete_blog(item_id)
