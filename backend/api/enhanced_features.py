...
from services.enhanced_features_service import get_enhanced_features_service
import logging
...
async def health_check():
    ...
    try:
        service = get_enhanced_features_service()
...
@router.post("/")
async def create_enhanced_features(
    data: Dict[str, Any] = Body({}, description="Data for creating enhanced_features"),
    ...
    try:
        ...
        service = get_enhanced_features_service()
        result = await service.create_enhanced_features(data)
...
@router.get("/")
async def list_enhanced_featuress(
    ...
    try:
        service = get_enhanced_features_service()
        result = await service.list_enhanced_featuress(
...
async def get_enhanced_features(
    item_id: str = Path(..., description="ID of enhanced_features"),
    current_user: dict = Depends(get_current_admin)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_enhanced_features_service()
        result = await service.get_enhanced_features(item_id)
...
@router.put("/{item_id}")
async def update_enhanced_features(
    item_id: str = Path(..., description="ID of enhanced_features"),
    ...
    try:
        ...
        service = get_enhanced_features_service()
        result = await service.update_enhanced_features(item_id, data)
...
async def delete_enhanced_features(
    item_id: str = Path(..., description="ID of enhanced_features"),
    current_user: dict = Depends(get_current_admin)
):
    ...
    try:
        service = get_enhanced_features_service()
        result = await service.delete_enhanced_features(item_id)
