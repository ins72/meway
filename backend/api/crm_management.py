...
from services.crm_management_service import get_crm_management_service
import logging
...
async def health_check():
    ...
    try:
        service = get_crm_management_service()
...
@router.post("/")
async def create_crm_management(
    data: Dict[str, Any] = Body({}, description="Data for creating crm_management"),
    ...
    try:
        ...
        service = get_crm_management_service()
        result = await service.create_crm_management(data)
...
@router.get("/")
async def list_crm_managements(
    ...
    try:
        service = get_crm_management_service()
        result = await service.list_crm_managements(
...
@router.get("/{item_id}")
async def get_crm_management(
    item_id: str = Path(..., description="ID of crm_management"),
    current_user: dict = Depends(get_current_admin)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_crm_management_service()
        result = await service.get_crm_management(item_id)
...
@router.put("/{item_id}")
async def update_crm_management(
    item_id: str = Path(..., description="ID of crm_management"),
    ...
    try:
        ...
        service = get_crm_management_service()
        result = await service.update_crm_management(item_id, data)
...
async def delete_crm_management(
    item_id: str = Path(..., description="ID of crm_management"),
    current_user: dict = Depends(get_current_admin)
):
    ...
    try:
        service = get_crm_management_service()
        result = await service.delete_crm_management(item_id)
