...
async def create_template(
    data: Dict[str, Any] = Body({}, description="Data for creating template"),
...
async def get_template(
    item_id: str = Path(..., description="ID of template"),
    current_user: dict = Depends(get_current_admin)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_template_service()
        result = await service.get_template(item_id)
...
async def update_template(
    ...
    try:
        ...
        service = get_template_service()
        result = await service.update_template(item_id, data)
...
async def delete_template(
    ...
    try:
        service = get_template_service()
        result = await service.delete_template(item_id)
