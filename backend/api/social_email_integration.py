...
from services.social_email_integration_service import get_social_email_integration_service
import logging
...
async def health_check():
    ...
    try:
        service = get_social_email_integration_service()
...
@router.post("/")
async def create_social_email_integration(
    data: Dict[str, Any] = Body({}, description="Data for creating social_email_integration"),
    ...
    try:
        ...
        service = get_social_email_integration_service()
        result = await service.create_social_email_integration(data)
...
@router.get("/")
async def list_social_email_integrations(
    ...
    try:
        service = get_social_email_integration_service()
        result = await service.list_social_email_integrations(
...
@router.get("/{item_id}")
async def get_social_email_integration(
    item_id: str = Path(..., description="ID of social_email_integration"),
    current_user: dict = Depends(get_current_user)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_social_email_integration_service()
        result = await service.get_social_email_integration(item_id)
...
@router.put("/{item_id}")
async def update_social_email_integration(
    item_id: str = Path(..., description="ID of social_email_integration"),
    ...
    try:
        ...
        service = get_social_email_integration_service()
        result = await service.update_social_email_integration(item_id, data)
...
async def delete_social_email_integration(
    item_id: str = Path(..., description="ID of social_email_integration"),
    current_user: dict = Depends(get_current_user)
):
    ...
    try:
        service = get_social_email_integration_service()
        result = await service.delete_social_email_integration(item_id)
