...
from services.template_marketplace_service import get_template_marketplace_service
import logging
...
async def health_check():
    ...
    try:
        service = get_template_marketplace_service()
...
@router.post("/")
async def create_template_marketplace(
    data: Dict[str, Any] = Body({}, description="Data for creating template_marketplace"),
    ...
    try:
        ...
        service = get_template_marketplace_service()
        result = await service.create_template_marketplace(data)
...
@router.get("/")
async def list_template_marketplaces(
    ...
    try:
        service = get_template_marketplace_service()
        result = await service.list_template_marketplaces(
...
async def get_template_marketplace(
    item_id: str = Path(..., description="ID of template_marketplace"),
    current_user: dict = Depends(get_current_admin)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_template_marketplace_service()
        result = await service.get_template_marketplace(item_id)
...
@router.put("/{item_id}")
async def update_template_marketplace(
    item_id: str = Path(..., description="ID of template_marketplace"),
    ...
    try:
        ...
        service = get_template_marketplace_service()
        result = await service.update_template_marketplace(item_id, data)
...
async def delete_template_marketplace(
    item_id: str = Path(..., description="ID of template_marketplace"),
    current_user: dict = Depends(get_current_admin)
):
    ...
    try:
        service = get_template_marketplace_service()
        result = await service.delete_template_marketplace(item_id)
