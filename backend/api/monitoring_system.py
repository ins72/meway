...
from services.monitoring_system_service import get_monitoring_system_service
import logging
...
async def health_check():
    ...
    try:
        service = get_monitoring_system_service()
...
async def create_monitoring_system(
    data: Dict[str, Any] = Body({}, description="Data for creating monitoring_system"),
    ...
    try:
        ...
        service = get_monitoring_system_service()
        result = await service.create_monitoring_system(data)
...
@router.get("/")
async def list_monitoring_systems(
    ...
    try:
        service = get_monitoring_system_service()
        result = await service.list_monitoring_systems(
...
async def get_monitoring_system(
    item_id: str = Path(..., description="ID of monitoring_system"),
    current_user: dict = Depends(get_current_admin)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_monitoring_system_service()
        result = await service.get_monitoring_system(item_id)
...
@router.put("/{item_id}")
async def update_monitoring_system(
    item_id: str = Path(..., description="ID of monitoring_system"),
    ...
    try:
        ...
        service = get_monitoring_system_service()
        result = await service.update_monitoring_system(item_id, data)
...
async def delete_monitoring_system(
    item_id: str = Path(..., description="ID of monitoring_system"),
    current_user: dict = Depends(get_current_admin)
):
    ...
    try:
        service = get_monitoring_system_service()
        result = await service.delete_monitoring_system(item_id)
