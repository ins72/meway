...
class MediaCreate(BaseModel):
    name: str = Field(..., min_length=1, max_length=255, description="Name of the media")
    description: Optional[str] = Field(None, max_length=1000, description="Description of the media")
    status: Optional[str] = Field("active", description="Status of the media")
...
class MediaUpdate(BaseModel):
    name: Optional[str] = Field(None, min_length=1, max_length=255, description="Name of the media")
    description: Optional[str] = Field(None, max_length=1000, description="Description of the media")
    status: Optional[str] = Field(None, description="Status of the media")
    metadata: Optional[Dict[str, Any]] = Field(None, description="Additional metadata")
...
async def health_check():
    """Comprehensive health check for media service"""
    try:
...
async def create_media(
    item: MediaCreate = Body(...),
    current_user: dict = Depends(get_current_user)
):
    """Create new media with comprehensive validation"""
    try:
        ...
        service = get_media_service()
        result = await service.create_media(item_data, user_id=user_id)
        ...
        logger.error(f"Create media failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def list_medias(
    ...
    """List medias with comprehensive filtering and pagination"""
    try:
        user_id = current_user.get("id") or current_user.get("user_id")
        ...
        if search:
            service = get_media_service()
            result = await service.search_medias(
                search_query=search,
                user_id=user_id,
                limit=limit,
                offset=offset
        ...
        logger.error(f"List medias failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@router.get("/{item_id}", response_model=MediaResponse)
async def get_media(
    item_id: str = Path(..., description="ID of the media to retrieve"),
    current_user: dict = Depends(get_current_user)
):
    """Get media by ID with comprehensive error handling"""
    try:
        user_id = current_user.get("id") or current_user.get("user_id")
        
        service = get_media_service()
        result = await service.get_media(item_id, user_id=user_id)
...
async def update_media(
    ...
    try:
        ...
        service = get_media_service()
        result = await service.update_media(item_id, item_data, user_id=user_id)
        ...
        logger.error(f"Update media failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))

# Delete Operation
@router.delete("/{item_id}", response_model=MediaResponse)
async def delete_media(
    item_id: str = Path(..., description="ID of the media to delete"),
    permanent: bool = Query(False, description="Permanent delete (true) or soft delete (false)"),
    current_user: dict = Depends(get_current_user)
):
    """Delete media with comprehensive validation"""
    try:
        user_id = current_user.get("id") or current_user.get("user_id")
...
# Statistics
@router.get("/stats", response_model=MediaResponse)
async def get_media_stats(
    current_user: dict = Depends(get_current_user)
):
    """Get comprehensive statistics for medias"""
    try:
        user_id = current_user.get("id") or current_user.get("user_id")
        ...
        logger.error(f"Get media stats failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def bulk_create_medias(
    ...
    try:
        ...
        service = get_media_service()
        result = await service.bulk_create_medias(items_data, user_id=user_id)
        ...
        logger.error(f"Bulk create medias failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))