...
from services.team_management_service import get_team_management_service
import logging
...
async def health_check():
    ...
    try:
        service = get_team_management_service()
...
@router.post("/")
async def create_team_management(
    data: Dict[str, Any] = Body({}, description="Data for creating team_management"),
    ...
    try:
        ...
        service = get_team_management_service()
        result = await service.create_team_management(data)
...
@router.get("/")
async def list_team_managements(
    ...
    try:
        service = get_team_management_service()
        result = await service.list_team_managements(
...
async def get_team_management(
    item_id: str = Path(..., description="ID of team_management"),
    current_user: dict = Depends(get_current_user)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_team_management_service()
        result = await service.get_team_management(item_id)
...
@router.put("/{item_id}")
async def update_team_management(
    item_id: str = Path(..., description="ID of team_management"),
    ...
    try:
        ...
        service = get_team_management_service()
        result = await service.update_team_management(item_id, data)
...
async def delete_team_management(
    item_id: str = Path(..., description="ID of team_management"),
    current_user: dict = Depends(get_current_user)
):
    ...
    try:
        service = get_team_management_service()
        result = await service.delete_team_management(item_id)
