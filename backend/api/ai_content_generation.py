...
async def health_check():
    ...
    try:
        service = get_ai_content_generation_service()
...
@router.post("/")
async def create_ai_content_generation(
    data: Dict[str, Any] = Body({}, description="Data for creating ai_content_generation"),
    ...
    try:
        ...
        service = get_ai_content_generation_service()
        result = await service.create_ai_content_generation(data)
...
@router.get("/")
async def list_ai_content_generations(
    ...
    try:
        service = get_ai_content_generation_service()
        result = await service.list_ai_content_generations(
...
@router.get("/{item_id}")
async def get_ai_content_generation(
    item_id: str = Path(..., description="ID of ai_content_generation"),
    current_user: dict = Depends(get_current_user)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_ai_content_generation_service()
        result = await service.get_ai_content_generation(item_id)
...
@router.put("/{item_id}")
async def update_ai_content_generation(
    item_id: str = Path(..., description="ID of ai_content_generation"),
    ...
    try:
        ...
        service = get_ai_content_generation_service()
        result = await service.update_ai_content_generation(item_id, data)
...
async def delete_ai_content_generation(
    item_id: str = Path(..., description="ID of ai_content_generation"),
    current_user: dict = Depends(get_current_user)
):
    ...
    try:
        service = get_ai_content_generation_service()
        result = await service.delete_ai_content_generation(item_id)
