...
async def create_admin(
    data: Dict[str, Any] = Body({}, description="Data for creating admin"),
    ...
    try:
        ...
        service = get_admin_service()
        result = await service.create_admin(data)
...
@router.get("/{item_id}")
async def get_admin(
    item_id: str = Path(..., description="ID of admin"),
    current_user: dict = Depends(get_current_admin)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_admin_service()
        result = await service.get_admin(item_id)
...
async def update_admin(
    ...
    try:
        ...
        service = get_admin_service()
        result = await service.update_admin(item_id, data)
...
async def delete_admin(
    ...
    try:
        service = get_admin_service()
        result = await service.delete_admin(item_id)
