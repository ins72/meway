...
class TeamCreate(BaseModel):
    ...
    description: Optional[str] = Field(None, max_length=1000, description="Description of the team")
    status: Optional[str] = Field("active", description="Status of the team")
...
class TeamUpdate(BaseModel):
    name: Optional[str] = Field(None, min_length=1, max_length=255, description="Name of the team")
    description: Optional[str] = Field(None, max_length=1000, description="Description of the team")
    status: Optional[str] = Field(None, description="Status of the team")
    metadata: Optional[Dict[str, Any]] = Field(None, description="Additional metadata")
...
async def health_check():
    """Comprehensive health check for team service"""
    try:
...
async def create_team(
    item: item: TeamCreate = Body(...),
    current_user: dict = Depends(get_current_user)
):
    """Create new team with comprehensive validation"""
    try:
        ...
        service = get_team_service()
        result = await service.create_team(item_data, user_id=user_id)
...
async def list_teams(
    ...
    """List teams with comprehensive filtering and pagination"""
    try:
        user_id = current_user.get("id") or current_user.get("user_id")
        ...
        if search:
            service = get_team_service()
            result = await service.search_teams(
                search_query=search,
                user_id=user_id,
                limit=limit,
                offset=offset
...
@router.get("/{item_id}", response_model=TeamResponse)
async def get_team(
    item_id: str = Path(..., description="ID of the team to retrieve"),
    current_user: dict = Depends(get_current_user)
):
    """Get team by ID with comprehensive error handling"""
    try:
        user_id = current_user.get("id") or current_user.get("user_id")
        
        service = get_team_service()
        result = await service.get_team(item_id, user_id=user_id)
...
async def update_team(
    ...
    try:
        ...
        service = get_team_service()
        result = await service.update_team(item_id, item_data, user_id=user_id)
        ...
        logger.error(f"Update team failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))

# Delete Operation
@router.delete("/{item_id}", response_model=TeamResponse)
async def delete_team(
    item_id: str = Path(..., description="ID of the team to delete"),
    permanent: bool = Query(False, description="Permanent delete (true) or soft delete (false)"),
    current_user: dict = Depends(get_current_user)
):
    """Delete team with comprehensive validation"""
    try:
        user_id = current_user.get("id") or current_user.get("user_id")
...
async def get_team_stats(
    ...
    """Get comprehensive statistics for teams"""
    try:
        user_id = current_user.get("id") or current_user.get("user_id")
...
# Bulk Operations
@router.post("/bulk", response_model=TeamResponse)
async def bulk_create_teams(
    items: items: List[TeamCreate] = Body(...),
    current_user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        service = get_team_service()
        result = await service.bulk_create_teams(items_data, user_id=user_id)
        ...
        logger.error(f"Bulk create teams failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))