...
from services.webhook_system_service import get_webhook_system_service
import logging
...
async def health_check():
    ...
    try:
        service = get_webhook_system_service()
...
@router.post("/")
async def create_webhook_system(
    data: Dict[str, Any] = Body({}, description="Data for creating webhook_system"),
    ...
    try:
        ...
        service = get_webhook_system_service()
        result = await service.create_webhook_system(data)
...
@router.get("/")
async def list_webhook_systems(
    ...
    try:
        service = get_webhook_system_service()
        result = await service.list_webhook_systems(
...
@router.get("/{item_id}")
async def get_webhook_system(
    item_id: str = Path(..., description="ID of webhook_system"),
    current_user: dict = Depends(get_current_admin)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_webhook_system_service()
        result = await service.get_webhook_system(item_id)
...
@router.put("/{item_id}")
async def update_webhook_system(
    item_id: str = Path(..., description="ID of webhook_system"),
    ...
    try:
        ...
        service = get_webhook_system_service()
        result = await service.update_webhook_system(item_id, data)
...
async def delete_webhook_system(
    item_id: str = Path(..., description="ID of webhook_system"),
    current_user: dict = Depends(get_current_admin)
):
    ...
    try:
        service = get_webhook_system_service()
        result = await service.delete_webhook_system(item_id)
