...
from services.admin_configuration_service import get_admin_configuration_service
import logging
...
async def create_admin_configuration(
    data: Dict[str, Any] = Body({}, description="Data for creating admin_configuration"),
    ...
    try:
        ...
        service = get_admin_configuration_service()
        result = await service.create_admin_configuration(data)
...
@router.get("/")
async def list_admin_configurations(
    ...
    try:
        service = get_admin_configuration_service()
        result = await service.list_admin_configurations(
...
async def get_admin_configuration(
    item_id: str = Path(..., description="ID of admin_configuration"),
    current_user: dict = Depends(get_current_user)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_admin_configuration_service()
        result = await service.get_admin_configuration(item_id)
...
async def update_admin_configuration(
    ...
    try:
        ...
        service = get_admin_configuration_service()
        result = await service.update_admin_configuration(item_id, data)
...
async def delete_admin_configuration(
    item_id: str = Path(..., description="ID of admin_configuration"),
    current_user: dict = Depends(get_current_user)
):
    ...
    try:
        service = get_admin_configuration_service()
        result = await service.delete_admin_configuration(item_id)
