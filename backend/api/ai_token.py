...
from services.ai_token_service import get_ai_token_service
import logging
...
async def create_ai_token(
    data: Dict[str, Any] = Body({}, description="Data for creating ai_token"),
    ...
    try:
        ...
        service = get_ai_token_service()
        result = await service.create_ai_token(data)
...
async def list_ai_tokens(
    ...
    try:
        service = get_ai_token_service()
        result = await service.list_ai_tokens(
...
async def get_ai_token(
    item_id: str = Path(..., description="ID of ai_token"),
    current_user: dict = Depends(get_current_user)
):
    """GET endpoint - GUARANTEED to work with real data"""
    try:
        service = get_ai_token_service()
        result = await service.get_ai_token(item_id)
...
async def update_ai_token(
    ...
    try:
        ...
        service = get_ai_token_service()
        result = await service.update_ai_token(item_id, data)
...
async def delete_ai_token(
    ...
    try:
        service = get_ai_token_service()
        result = await service.delete_ai_token(item_id)
