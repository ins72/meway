...
class AutomationService:
    ...
    async def list_automations(self, user_id: str = None, limit: int = 50, offset: int = 0) -> Dict[str, Any]:
        """List automations"""
        try:
            db = self._get_db()
            if not db:
    ...
    async def update_automation(self, item_id: str, update_data: Dict[str, Any]) -> Dict[str, Any]:
        """Update automation"""
        try:
            db = self._get_db()
            if not db:
    ...
    async def delete_automation(self, item_id: str) -> Dict[str, Any]:
        """Delete automation"""
        try:
            db = self._get_db()
            if not db:
