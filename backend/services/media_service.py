...
class MediaService:
    ...
    async def create_media(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Create new media"""
        try:
            db = self._get_db()
            if not db:
    ...
    async def get_media(self, item_id: str) -> Dict[str, Any]:
        """Get media by ID"""
        try:
            db = self._get_db()
            if not db:
    ...
    async def list_medias(self, user_id: str = None, limit: int = 50, offset: int = 0) -> Dict[str, Any]:
        """List medias"""
        try:
            db = self._get_db()
            if not db:
    ...
    async def update_media(self, item_id: str, update_data: Dict[str, Any]) -> Dict[str, Any]:
        """Update media"""
        try:
            db = self._get_db()
            if not db:
    ...
    async def delete_media(self, item_id: str) -> Dict[str, Any]:
        """Delete media"""
        try:
            db = self._get_db()
            if not db:
