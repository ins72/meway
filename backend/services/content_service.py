...
class ContentService:
    ...
    async def list_contents(self, user_id: str = None, limit: int = 50, offset: int = 0) -> Dict[str, Any]:
        """List contents"""
        try:
            db = self._get_db()
            if not db:
    ...
    async def update_content(self, item_id: str, update_data: Dict[str, Any]) -> Dict[str, Any]:
        """Update content"""
        try:
            db = self._get_db()
            if not db:
    ...
    async def delete_content(self, item_id: str) -> Dict[str, Any]:
        """Delete content"""
        try:
            db = self._get_db()
            if not db:
